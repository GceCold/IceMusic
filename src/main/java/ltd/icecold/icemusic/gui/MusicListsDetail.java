package ltd.icecold.icemusic.gui;
...
import ltd.icecold.icemusic.command.subcommands.Play;
import ltd.icecold.icemusic.config.Login;
import ltd.icecold.icemusic.config.gui.ListDetail;
import ltd.icecold.icemusic.gui.bean.ListDetailResultBean;
    ...
    public static void openGui(Player player, String type, Long id, String name) {
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    MessageService messageService = (MessageService) IceMusic.nettyClient.getBean(IceMusic.getInstance().getClass().getClassLoader(),MessageService.class,"#IceMusic_ICECOLD#MUSIC_LIST_MUSIC_LIST_DETAIL");
                    ...
                    if (result.isEmpty() || "error".equals(result)) {
                        player.sendMessage("§7[IceMusic] >§c服务器内部发生错误！错误代码:§l§eERROR005");
                    ...
                    if (!new JsonParser().parse(result).getAsJsonObject().has("data") || !new JsonParser().parse(result).getAsJsonObject().has("code")) {
                        player.sendMessage("§7[IceMusic] >§c服务器内部发生错误！错误代码:§l§eERROR006");
                        player.closeInventory();
                        cancel();
                        return;
                    }
            ...
            }.runTaskAsynchronously(IceMusic.getInstance());
    }
}
