...
import ltd.icecold.icemusic.config.gui.MusicResult;
import ltd.icecold.icemusic.gui.bean.SearchMusicResultBean;
...
public class ResultMusicGui {
    public static void openSearchMusicGui(Player player, String musicName) {
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (musicList == null) {
                    player.sendMessage("§7[IceMusic] >§c未搜索到歌曲：" + musicName);
                    cancel();
                    return;
                }
    ...
    public static Map<String, Integer> getMusicInfo(Player player, String musicName) {
        ...
        if (result == null) {
            player.sendMessage("§7[IceMusic] >§c歌曲信息查询失败");
            return null;
        }
        String code = new JsonParser().parse(result).getAsJsonObject().get("code").getAsString();
        if (!"200".equals(code)) {
            player.sendMessage("§7[IceMusic] >§c歌曲信息查询失败，未找到歌曲");
            return null;
        }
