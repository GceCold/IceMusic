...
import ltd.icecold.icemusic.config.gui.Search;
        ...
        VexButton vexButton = new VexButton(
                ...
                player1 -> {
                    if (vextextfield.getTypedText().equals("")){
                        player1.sendMessage("§7[IceMusic§7] §e> 请输入查询名称");
                        return;
                    }
                    if (checkBoxMusic.isChecked() && checkBoxPlayList.isChecked()){
                        player1.sendMessage("§7[IceMusic§7] §e> 您只能选择一个");
                        return;
                    }else if (!checkBoxMusic.isChecked() && !checkBoxPlayList.isChecked()){
                        player1.sendMessage("§7[IceMusic§7] §e> 请选择一个查询类别");
                        return;
                    }
